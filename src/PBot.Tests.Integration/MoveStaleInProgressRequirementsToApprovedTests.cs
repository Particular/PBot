...
public class MoveStaleInProgressRequirementsToApprovedTests : GitHubIntegrationTest
{
    ...
    {
        var staleInProgressCard = await GitHubClient.Issue.Create(RepositoryOwner, RepositoryName, Requirements.NewFeature("InProgress Stale", state: RequirementStates.InProgress));
